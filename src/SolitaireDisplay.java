...
public class SolitaireDisplay extends JComponent implements MouseListener
{
  ...
  public void paintComponent(Graphics g)
  {
    ...
    //Rectangle score
    g.drawRect(25, getHeight()-50, 100, 25);
    g.drawString("Score: " + score, 50, getHeight()-33);

    //Partial move
    g.drawRect(getWidth()/2 - 65, getHeight()-50, 175, 25);
    if(game.getToggle())
      g.drawString("Partial Move Toggle: On", getWidth()/2-50, getHeight()-33);
    else
      g.drawString("Partial Move Toggle: Off", getWidth()/2-50, getHeight()-33);


    //New Game
    g.drawRect(getWidth()-115, getHeight()-50, 100, 25);
    g.drawString("New Game", getWidth()-100, getHeight()-33);
  }
  ...
  public void mouseClicked(MouseEvent e)
  {
    ...
    int rowWithPixels = e.getY();
    int colWithPixels = e.getX();
    ...
    if (row == 0 && col == 0)
      game.stockClicked();
    else if (rowWithPixels > getHeight() - 50 && colWithPixels < getWidth()/2 + 103 && colWithPixels > getWidth()/2 - 50)
      game.partialCheatClicked();
    else if (row == 0 && col == 1)
      game.wasteClicked();
    else if (row == 0 && col >= 3)
      game.foundationClicked(col - 3);
    else if (rowWithPixels > 78 && rowWithPixels < 50 + 3*CARD_HEIGHT)
      game.pileClicked(col);
    else if (rowWithPixels > 4*CARD_HEIGHT && colWithPixels > getWidth()/2 + 155)
      game.newGameClicked();
    repaint();
  }
  ...
  {
    score += rank;
    repaint(25, getHeight()-50, 100, 25);
  }
